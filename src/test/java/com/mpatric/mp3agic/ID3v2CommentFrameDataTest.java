...
public class ID3v2CommentFrameDataTest extends TestCase {
	...
	public void testShouldConsiderTwoEquivalentObjectsEqual() throws Exception {
		ID3v2CommentFrameData frameData1 = new ID3v2CommentFrameData(false, TEST_LANGUAGE, 
				new EncodedText(TEST_DESCRIPTION), 
				new EncodedText(TEST_VALUE));
		ID3v2CommentFrameData frameData2 = new ID3v2CommentFrameData(false, TEST_LANGUAGE, 
				new EncodedText(TEST_DESCRIPTION),
				new EncodedText(TEST_VALUE));
	...
	public void testShouldConvertFrameDataToBytesAndBackToEquivalentObject() throws Exception {
		ID3v2CommentFrameData frameData = new ID3v2CommentFrameData(false, TEST_LANGUAGE, new EncodedText(TEST_DESCRIPTION), new EncodedText(TEST_VALUE));
	...
	public void testShouldConvertFrameDataWithUnicodeToBytesAndBackToEquivalentObject() throws Exception {
		ID3v2CommentFrameData frameData = new ID3v2CommentFrameData(false, TEST_LANGUAGE, new EncodedText(Encoding.ENCODING_UTF_16, TEST_DESCRIPTION_UNICODE), new EncodedText(Encoding.ENCODING_UTF_16, TEST_VALUE_UNICODE));
		byte[] bytes = frameData.toBytes();
