...
public class Mp3File {
    ...
    public Mp3File(byte[] mediaContent) throws IOException, UnsupportedTagException, InvalidDataException {
        this(mediaContent, DEFAULT_BUFFER_LENGTH, true);
    }

    public Mp3File(byte[] mediaContent, int bufferLength) throws IOException, UnsupportedTagException, InvalidDataException {
        this(mediaContent, bufferLength, true);
    }
    ...
    private void sanityCheckFrame(MpegFrame frame, int offset) throws InvalidDataException {
        ...
        if (!version.equals(frame.getVersion()))
            throw new InvalidDataException("Inconsistent frame header");
        if (offset + frame.getLengthInBytes() > mediaSource.getLength())
    ...
    /**
     * Returns either ID3 v1 or v2 if it exists. ID3v2 takes preference over ID3v1.
     * 
     * @return id3v2 if present and id3v1 if not
     */
    public Object getId3Tag() {
        if (hasId3v2Tag()) {
            return id3v2Tag;
        }
        return id3v1Tag;
