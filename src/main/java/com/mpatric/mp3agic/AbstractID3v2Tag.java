...
public abstract class AbstractID3v2Tag implements ID3v2 {
    ...
    public List<String> getUrls() {
        ...
        if (frameData != null) {
            ...
            for (ID3v2UrlFrameData frame : frameData) {
                urls.add(frame.getUrl());
            }
            return urls;
        }
        return null;
    ...
    public void setAlbumImage(byte[] albumImage, String mimeType, byte pictureType) {
        if (albumImage != null && albumImage.length > 0 && mimeType != null && mimeType.length() > 0) {
            invalidateDataLength();
            ID3v2PictureFrameData frameData = new ID3v2PictureFrameData(useFrameUnsynchronisation(), mimeType, pictureType, null, albumImage);
            addFrame(createFrame(ID_IMAGE, frameData.toBytes()), true);
        }
    }

    public void setAlbumImage(byte[] albumImage, String mimeType) {
        setAlbumImage(albumImage, mimeType, (byte) 0);
    }
    ...
    private ArrayList<ID3v2UrlFrameData> extractUrlFrameData(String id) {
        ID3v2FrameSet frameSet = frameSets.get(id);
        if (frameSet != null) {
            List<ID3v2Frame> frames = frameSet.getFrames();
            ArrayList<ID3v2UrlFrameData> urlData = new ArrayList<>(frames.size());
            for (ID3v2Frame frame : frames) {
                ID3v2UrlFrameData frameData;
                try {
                    frameData = new ID3v2UrlFrameData(useFrameUnsynchronisation(), frame.getData());
                    urlData.add(frameData);
                } catch (InvalidDataException e) {
                    // do nothing
                }
            }
