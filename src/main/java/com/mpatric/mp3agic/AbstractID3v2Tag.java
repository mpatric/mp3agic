...
public abstract class AbstractID3v2Tag implements ID3v2 {
    ...
    private int unpackExtendedHeader(byte[] bytes, int offset) {
        extendedHeaderLength = BufferTools.unpackSynchsafeInteger(bytes[offset], bytes[offset + 1],
                bytes[offset + 2], bytes[offset + 3]) + 4;
            ...
            if ((onlyId == null || onlyId.equals(frameSet.getId()))
                ...
                while (frameIterator.hasNext()) {
                    ...
                    if (frame.getDataLength() > 0) {
                        ...
                        BufferTools.copyIntoByteBuffer(frameData, 0, frameData.length, bytes,
                                offset);
    ...
    public void setItunesComment(String itunesComment) {
        if (itunesComment != null && itunesComment.length() > 0) {
            invalidateDataLength();
            ID3v2CommentFrameData frameData = new ID3v2CommentFrameData(
                    useFrameUnsynchronisation(), ITUNES_COMMENT_DESCRIPTION, null, new EncodedText(
                            itunesComment));
            addFrame(createFrame(ID_COMMENT, frameData.toBytes()), true);
        }
    }
    ...
    public void setAlbumImage(byte[] albumImage, String mimeType) {
        if (albumImage != null && albumImage.length > 0 && mimeType != null
            ...
            ID3v2PictureFrameData frameData = new ID3v2PictureFrameData(
                    useFrameUnsynchronisation(), mimeType, (byte) 0, null, albumImage);
    ...
    private ID3v2CommentFrameData extractCommentFrameData(String id, boolean itunes) {
        ...
        if (frameSet != null) {
            ...
            while (iterator.hasNext()) {
                ...
                ID3v2CommentFrameData frameData;
                try {
                    frameData = new ID3v2CommentFrameData(useFrameUnsynchronisation(),
                            frame.getData());
                    if (itunes
                            && ITUNES_COMMENT_DESCRIPTION.equals(frameData.getDescription()
                                    .toString())) {
                        return frameData;
                    } else if (!itunes) {
                        return frameData;
        ...
        if (frameSet != null) {
            ...
            try {
                if (obseleteFormat)
                    ...
                    frameData = new ID3v2PictureFrameData(useFrameUnsynchronisation(),
                            frame.getData());
                return frameData;
            } catch (InvalidDataException e) {
                // do nothing
